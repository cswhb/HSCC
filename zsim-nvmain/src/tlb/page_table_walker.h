...
class PageTableWalker: public BasePageTableWalker
{
	...
	private:
		...
		inline void evict_and_fetch_out(MemReq& req, DRAMBufferBlock* dram_block,T* entry, bool& evict){
			...
			index=(dram_block->block_id>>order)<<order;
			for(unsigned i=0;i<(1<<order);i++){
				block=zinfo->dram_manager->get_page_ptr(index+i);
				block->validate(((req.lineAddr>>(zinfo->block_shift)>>order)<<order)+i);//1
			    block->set_src_addr( ((entry->p_page_no>>order)<<order)+i );//1
			    block->set_vaddr( ((entry->v_page_no>>order)<<order)+i );//1
